...
function GENERATE_HEMMING_CODE(inputText) {
    ...
    if (inputText.length !== 16) return "Message must be 16 characters without spaces";
  ...
  function _findErrorPosition(k_bits, arr) {
    const res = "" + ((k_bits[16] + arr[16]) % 2) + ((k_bits[8] + arr[8]) % 2) + ((k_bits[4] + arr[4]) % 2) + ((k_bits[2] + arr[2]) % 2) + ((k_bits[1] + arr[1]) % 2);
