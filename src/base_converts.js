...
const HEX2BIN_ONLY_INT = (hex) => Array.from(hex).map(hexDigit => parseInt(hexDigit, 16).toString(2).padStart(4, '0')).join('');
...
const _validateInput = (input, delimiter) => typeof input === 'string' && input.includes(delimiter);
...
const _decToBaseExtended = (decString, base) => {
  ...
  const fractional = _convertFractional(`0.${fractionalPart}`, base, MAX_FRACTION_DIGITS);
...
const _hexToBaseExtended = (hexString, base) => {
  ...
  const fractional = parseInt(fractionalPart, 16) / Math.pow(16, fractionalPart.length);
  const fractionalBase = _convertFractional(fractional, base, MAX_FRACTION_DIGITS);
